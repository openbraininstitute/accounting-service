...
async def _insert_job(db, job_id, instances, started_at):
    await db.execute(
        sa.insert(Job),
        [
            {
...
async def test_charge_longrun(db):
    ...
    # no jobs
    result = await test_module.charge_longrun(repos)
    ...
    job_id = create_uuid()
    now = utcnow()
    await _insert_job(db, job_id, instances=1, started_at=now)
