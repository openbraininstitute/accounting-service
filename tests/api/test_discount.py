...
@pytest.mark.usefixtures("_db_account")
async def test_update_discount(api_client):
    discount_dict = {
        "vlab_id": VLAB_ID,
        "discount": "0.2",
    ...
    created_discount = res.json()["data"]
    ...
    response = await api_client.put("/discount", json=updated_account_dict)

    assert response.status_code == 200

    updated_discount = response.json()["data"]

    assert updated_discount["id"] == created_discount["id"]
    assert updated_discount["vlab_id"] == VLAB_ID
    assert updated_discount["discount"] == "0.4"


@pytest.mark.usefixtures("_db_account")
async def test_update_discount_with_non_existent_vlab_id(api_client):
    discount_dict = {
        "vlab_id": VLAB_ID,
        "discount": "0.2",
        "valid_from": str(datetime.now(UTC)),
...
@pytest.mark.usefixtures("_db_account")
async def test_update_discount_with_incorrect_valid_to(api_client):
    discount_dict = {
        "vlab_id": VLAB_ID,
        "discount": "0.2",
        "valid_from": str(datetime.now(UTC)),
