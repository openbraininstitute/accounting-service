...
async def test_make_short_jobs_reservation(api_client, mock_uuid):
    ...
    request_payload = {
        "proj_id": PROJ_ID,
        "type": ServiceType.SHORT_JOBS,
        "subtype": "ml_queries",
        "count": "1",
    }
    response = await api_client.post("/api/v1/reservation/short_jobs", json=request_payload)
...
@pytest.mark.usefixtures("_db_account")
async def test_make_long_jobs_reservation(api_client, mock_uuid):
    # make a valid reservation
    request_payload = {
        "proj_id": PROJ_ID,
        "type": ServiceType.LONG_JOBS,
        "subtype": "single_cell_sim",
        "instances": "1",
        "instance_type": "",
    }
    response = await api_client.post("/api/v1/reservation/long_jobs", json=request_payload)
